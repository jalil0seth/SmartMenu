...
class SettingController extends Controller
{
    ...
    public function store(StoreSettingRequest $request)
    {
        ...
        foreach ($request->input('banners', []) as $file) {
            $setting->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('banners');
        }
    ...
    public function update(UpdateSettingRequest $request, Setting $setting)
    {
        ...
        if (count($setting->banners) > 0) {
            foreach ($setting->banners as $media) {
                if (!in_array($media->file_name, $request->input('banners', []))) {
                    $media->delete();
                }
            }
        }
        $media = $setting->banners->pluck('file_name')->toArray();
        foreach ($request->input('banners', []) as $file) {
